"""Verify real Stage17 reproducibility, all dashboard pages and immutable prior outputs."""
from __future__ import annotations

import argparse
import json
from pathlib import Path
import sys
from unittest.mock import patch

ROOT = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(ROOT))


def verify(baseline: Path | None = None) -> dict:
    """Block all requests; rerun Stage17 only and validate output hashes, reports and UI."""
    from streamlit.testing.v1 import AppTest
    from src.common_period_spatial.data import MANIFEST, REPORT_DIR, snapshot
    from src.common_period_spatial.workflow import run_analysis
    from src.common_period_spatial.artifacts import report_from_saved
    from src.spatial.robustness_audit import sha256, check_snapshot
    from src.nationwide.tier_a_pipeline import _atomic_json
    before = json.loads((ROOT/MANIFEST).read_text())
    protected = json.loads(baseline.read_text()) if baseline else snapshot(ROOT)
    with patch('requests.sessions.Session.request', side_effect=AssertionError('Stage17 verification forbids HTTP')) as http:
        dry = run_analysis(ROOT, dry_run=True)
        after = run_analysis(ROOT)
        changes = [name for name, digest in before['output_hashes'].items() if sha256(ROOT/name) != digest]
        reports = {name: sha256(ROOT/name) for name in after['output_files'] if '/reports/' in name}
        report_from_saved(ROOT)
        reports_equal = all(sha256(ROOT/name) == digest for name, digest in reports.items())
        pages = {}
        for path in sorted((ROOT/'dashboard/pages').glob('*.py')):
            if path.stem.startswith('_'):
                continue
            app = AppTest.from_string(f'from dashboard.pages.{path.stem} import render\nrender()').run(timeout=60)
            errors = [str(e.value) for e in app.exception]
            if path.stem == 'common_period_spatial' and not errors:
                for mode in ['Unique-grid', 'Station-linked']:
                    app.selectbox(key='stage17_representation').set_value(mode).run(timeout=60)
                    errors += [str(e.value) for e in app.exception]
                for metric in ['nasa_33c_sen_slope', 'nasa_25c_sen_slope', 'nasa_tmin_sen_slope']:
                    app.selectbox(key='stage17_nasa_metric').set_value(metric).run(timeout=60)
                    errors += [str(e.value) for e in app.exception]
                app.selectbox(key='stage17_kma_metric').set_value('tavg_bias').run(timeout=60)
                errors += [str(e.value) for e in app.exception]
            pages[path.stem] = {'passed': not errors, 'exceptions': errors}
            print(f'{path.stem}: {"PASS" if not errors else "FAIL"}', flush=True)
        router = AppTest.from_file(str(ROOT/'streamlit_app.py')).run(timeout=60)
        calls = http.call_count
    changed = check_snapshot(ROOT, protected)
    result = {'compared_artifacts': len(before['output_hashes']), 'changed_artifacts': changes,
              'report_regeneration_identical': reports_equal, 'page_count': len(pages), 'dashboard_pages': pages,
              'default_router_passed': not bool(router.exception), 'protected_files_count': len(protected),
              'protected_files_changed': changed, 'NASA_API_calls': 0, 'KMA_API_calls': 0,
              'http_request_attempts': calls, 'http_requests_blocked': True,
              'dry_run_station_count': dry['station_count'], 'dry_run_grid_count': dry['unique_NASA_grid_count'],
              'completed': not changes and reports_equal and not changed and not bool(router.exception)
              and all(p['passed'] for p in pages.values()) and calls == 0}
    _atomic_json(result, ROOT/REPORT_DIR/'common_period_spatial_verification.json')
    return result


def main() -> int:
    """Exit nonzero on any protection, reproducibility or rendering failure."""
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument('--baseline', type=Path)
    result = verify(parser.parse_args().baseline)
    print(json.dumps(result, ensure_ascii=False, indent=2))
    return 0 if result['completed'] else 1


if __name__ == '__main__':
    raise SystemExit(main())
